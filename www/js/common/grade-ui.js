...
/**
 * Register all grade UI handlers
 * Must be called after Shiny is available
 */
function registerGradeUIHandlers() {
    if (typeof Shiny === 'undefined' || !Shiny) {
        console.warn('Shiny not available yet, retrying in 100ms...');
        setTimeout(registerGradeUIHandlers, 100);
        return;
    }
    ...
    try {
        ...
        console.log('Grade UI handlers registered successfully');
    } catch (e) {
        console.error('Error registering grade UI handlers:', e);
    }
}

// Register handlers after DOM is ready
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', registerGradeUIHandlers);
} else {
    registerGradeUIHandlers();
}
